...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--include-anime-lama",
        action="store_true",
        help="also install the AnimeLaMa model (TorchScript Big LaMa)",
    )
    parser.add_argument(
        "--all",
        action="store_true",
        help="install all available models (CTD, AOT, AnimeLaMa)",
    )
...
def main() -> None:
    args = parse_args()
    if not args.baseline and not args.include_anime_lama and not args.all:
        raise SystemExit("select --baseline, --include-anime-lama, or --all")
    ...
    model_ids: list[str] = []
    if args.all:
        model_ids.extend(("ctd-onnx", "aot-onnx", "anime-lama"))
    else:
        if args.baseline:
