...
2. **Confirm where it goes** (default: Print-only output for user). Other valid destinations: PR description, `docs/postmortems/<ticket>.md`, internal wiki page.
3. **Produce the draft** as a single chat block.
4. **Deliver output.** Output the draft directly to the user so they can copy and paste it into their issue tracker.
...
- **Deliver output to the user.** Print-only output needs no approval. Do not attempt to automatically post to issue trackers unless specifically instructed with API details.
