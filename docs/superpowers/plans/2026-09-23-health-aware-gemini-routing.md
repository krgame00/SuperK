...
- **VERIFIED WORKING in deterministic tests:** Opt-in NDJSON model-switch progress flows from the actual next-model attempt through the image API to the active workspace hook. Legacy JSON clients remain unchanged. The fixed rollback switch, comma-separated key pool, and upstream-key redaction each have focused regression coverage.
- **VERIFIED WORKING in a credentialed local API sample:** `public/live_full_comparison.png` succeeded twice through health-aware Auto (5 bubbles; 46.4s and 18.2s), once through fixed Manual (5 bubbles; 32.5s), and once through fixed Auto (7 bubbles; 44.6s). The sample did not trigger a safety response. The fixed Auto path initially failed with HTTP 400 because the comma-separated server key pool was treated as one key; splitting and de-duplication repaired it before the successful run.
- **EXPERIMENTAL for release:** The original four failing pages have not been identified in the repository. Re-run those exact pages through the browser workflow and compare Auto, safety, and timing against the fixed rollback before claiming the production regression is closed.
- **Additional live evidence:** `public/comparison/compare_page_2.jpg` produced one Auto `GEMINI_TIMEOUT` at the 60-second budget, then Manual succeeded in 19.5s and Auto succeeded in 22.6s with two streamed model-switch events. `compare_page_34.jpg` succeeded in Auto in 29.8s. Treat the first timeout as a recorded transient until a repeatable failure path is established.
- **Final automated gate:** `vitest run --root .` passed **140 files / 883 tests** after the stream correction. TypeScript and focused routing/API lint passed. Exact four-page user workload and browser overlay acceptance remain pending.
- **Stream interruption correction:** A red test showed that a stream ending before its terminal result was a generic error. The response reader now reports a structured retryable `NETWORK` error while preserving explicit user cancellation. Focused stream/API/hook tests passed **59/59** and TypeScript passed.
